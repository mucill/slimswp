...
function slims_page() {
      ...
      if(isset($_GET['id'])) {
			...
			if (!$xml_read) { 
				echo 'Ooops...cannot load remote URL. Please check your configuration';
				exit;
			}
            ...
            foreach($xml->mods as $coll) {
				...
				if($slims->image != '') {
					  $img = '<div class="cover-list"><img src="'.$base_url.'lib/phpthumb/phpThumb.php?src=../../images/docs/'.$slims->image.'&w=190" /></div>';
				} else {
					$img = '';
				}
